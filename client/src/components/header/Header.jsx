...
const Header = () => {
    ...
    return (
        <header ref={headerRef} className="backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/95 shadow-sm sticky top-0 z-50">
            <nav
                className="container px-4 md:px-[50px]"
            >
                <div className=" flex items-center justify-between gap-3 md:gap-14 w-full flex-col md:flex-row sm:flex-row lg:flex-row py-2">
                    {/* primary div */}
                        ...
                        <div className=" flex gap-[20px] items-center w-[100%] flex-col md:flex-row sm:flex-row lg:flex-row">
                            ...
                            <Link to="/" className="shrink-0">
                                <img
                                    src={logo}
                                    alt="logo"
                                    className="max-h-fit hover:opacity-90 transition-opacity"
                                />
                            </Link>
                    ...
                    {/* secondary div */}
                    <div className="flex items-center justify-between gap-[30px] md:gap-[50px] w-[70%] mb-4 md:mb-0">
                        {/* home */}
                        <div className="flex items-center group">
                            <NavLink to="/" className="flex items-center gap-1 hover:text-slate-700 transition-colors">
                        ...
                        >
                            {auth.user ? (
                                <div className="flex items-center gap-1 ">
                                    ...
                                    <span>
                                        <RiArrowDropDownLine className="group-hover:rotate-[180deg] transition-transform " />
                                    </span>
                                </div>
                            ) : (
                            ...
                            {isDropdownOpen && (
                                <div
                                    className="absolute top-[36px] -left-[2px] z-50 bg-white/95 backdrop-blur border border-gray-200 shadow-lg rounded-md p-2 w-[160px] transition-all flex flex-col "
                                    onMouseEnter={toggleDropdown}
                                    onMouseLeave={closeDropdown}
                                >
                                    <ul>
                        ...
                        {auth?.user?.role !== 1 && (
                            <div className="flex items-center gap-1 group">
                                <NavLink
                                    to="/cart"
                                    className="relative flex items-center gap-1 hover:text-slate-700 transition-colors"
                                >
